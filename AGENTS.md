...
완료된 단계는 `main`에 반영된 상태를 기준으로 한다. 1단계부터 11단계까지 완료됐고, 11단계는 `review-stage-11-v1` 승인 후 `main`에 반영됐다. 다음 단계 작업은 승인된 최신 `main`에서 별도 브랜치를 만들어 수행하며, 12단계 부대 지정·단축키 기능은 별도 승인 명령 전까지 구현하지 않는다.
