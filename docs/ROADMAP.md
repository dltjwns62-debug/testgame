...
- `main` 반영 여부: 1단계부터 14단계까지 반영 완료, 15단계 미반영

현재 단계 번호는 15이며, 1단계부터 14단계까지 `main`에 반영되어 완료됐다. 15단계는 `stage-15-save-offline-progress`에서 검수 대기 중이며, 16단계는 시작하지 않는다.
...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 15이며, 1단계부터 14단계까지 `main` 반영이 완료됐다. 15단계는 `review_pending` 상태이고, 16단계는 시작하지 않는다.
...
`review_pending` — `review-stage-15-v3` 재제출, `main` 미반영
