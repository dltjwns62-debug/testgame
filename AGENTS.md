...
8단계에서는 BattleScene 내부 Auto Hunt ON/OFF와 수동 명령 우선순위만 구현한다. 월드맵 자동 몬스터 선택, A 키 공격 이동, 스킬, 상점, 편성 재배치, 부대 지정, 경험치·레벨·아이템·저장·온라인 기능은 구현하지 않는다. 기존 `favicon` 문제는 별도 작업 브랜치에서 검수한다.
