...
- 현재 작업: 8단계 동료 지원 타깃 기억 수정 완료 — `review-stage-08-v3` 검수 대기 중
...
8단계 v2의 지역 동료 지원·근접 사거리·공격 범위 표시를 유지하면서, 지원 아군 배정 시 `lastAttackerId`와 `lastAttackedAt`을 초기화하도록 수정했다. 실제 피해를 받은 아군 본인의 반격 기억은 유지하고, 분산된 지원 아군이 최초 공격자 기억 때문에 한 적에게 재집결하지 않도록 했다.
...
- 8단계 v3 지원 대상 사망 후 분산 재탐색: 사용자 수동 테스트 필요
