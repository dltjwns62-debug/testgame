...
완료된 단계는 `main`에 반영된 상태를 기준으로 한다. 1단계부터 14단계까지 완료됐고, 14단계 승인 작업은 `main`에 반영됐다. 다음 15단계 저장과 오프라인 진행 기능은 별도 시작 명령 전까지 구현하지 않는다.
