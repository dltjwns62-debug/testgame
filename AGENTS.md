...
완료된 단계는 `main`에 반영된 상태를 기준으로 한다. 다음 단계 작업 브랜치는 승인된 최신 `main`에서 생성한다. 1단계부터 5단계까지 완료됐고, 6단계 시작 명령 전에는 6단계를 시작하지 않는다.

5단계에서는 BattleScene의 기본 자동 공격, 고정 피해량, HP 변화, HP 바, 공격 로그, HP 0 전투 정지만 구현했다. 승리·패배·보상·몬스터 재생성은 6단계 전까지 구현하지 않는다. 필드 상단 단계 안내가 `Stage 4`로 남은 문제는 `main`에서 직접 수정하지 않고 6단계 작업 브랜치에서 수정한다. 다음 단계 작업은 별도 시작 명령 후 승인된 최신 `main`에서 생성한 작업 브랜치에서만 수행한다.
