...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 7이며, 현재 7단계는 `review_pending` 상태다. 1단계부터 6단계까지 `main` 반영이 완료됐고, 7단계는 작업 브랜치에서 검수 대기 중이다. 7단계 승인 전에는 `main`에 반영하지 않으며, 8단계 시작 명령 전에는 8단계를 시작하지 않는다.
...
`review_pending` — 구현 완료, 사용자 실행 테스트 및 ChatGPT 코드 검수 대기. 작업 브랜치: `stage-07-rts-battle-core`, 재검수 태그: `review-stage-07-v3`
