...
완료된 단계는 `main`에 반영된 상태를 기준으로 한다. 1단계부터 6단계까지 완료됐으며, 다음 단계 작업 브랜치는 승인된 최신 `main`에서 생성한다. 별도의 7단계 시작 명령 전에는 7단계 기능을 구현하지 않는다.

6단계에서는 BattleScene의 승리·패배 결과, 중복 없는 Gold 보상, 처치 몬스터의 3초 후 재생성, 패배 시 몬스터 유지, 필드 단계 문구 수정을 완료했다. 경험치·레벨·능력치·아이템·인벤토리·저장·온라인 기능은 아직 구현하지 않는다. `favicon` 문제를 수정할 경우 다음 작업 브랜치에서 별도 검수한다.
