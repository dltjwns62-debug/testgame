...
17단계까지의 전체 로드맵과 `stage-17-completed` 상태는 완료로 유지한다. 이 작업은 번호가 매겨진 18단계가 아니라 `post-roadmap-visual-ui-pass-1`에서 진행하는 별도 시각/UI 후속 패스이며, 현재 검수 태그는 `review-visual-ui-pass-1-v1`이다. `main`에는 아직 반영하지 않았고 사용자 수동 테스트는 `skipped_by_user`로 기록한다.
...
- 검수 상태: `review_pending` / `review-visual-ui-pass-1-v1`
