...
8단계에서는 전투 화면의 Auto Hunt ON/OFF, 전역 자동 타깃 탐색, 수동 MOVE·FOCUS_ATTACK 우선순위와 세션 내 설정 유지만 구현한다. 9단계 스킬과 다른 후속 기능은 아직 시작하지 않는다.
...
7단계 사용자 실행 테스트는 통과했고 ChatGPT 코드 검수는 승인됐다. 8단계는 현재 `review_pending` 상태로 검수 대기 중이다.
