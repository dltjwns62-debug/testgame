...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 3이며, 현재 3단계는 `review_pending` 상태다. 1단계와 2단계는 `main` 반영까지 완료됐고, 4단계는 `not_started` 상태로 유지한다. 3단계 승인 전에는 4단계를 시작하지 않는다.
...
1단계와 2단계의 모든 완료 조건이 충족되어 상태를 `completed`로 확정했다. 현재 3단계는 별도 작업 브랜치에서 검수 대기 중이며, 완료 기준은 검수 승인 후 `main` 반영이다.
...
자동 검사와 브라우저 상호작용 확인은 완료했지만 사용자 실행 테스트, ChatGPT 코드 검수, `main` 반영이 끝나지 않았으므로 3단계를 `completed`로 기록하지 않는다.

## 4단계: 플레이어와 몬스터 접촉 시 전투 화면 전환

### 단계 상태

`not_started` — 3단계 승인 후 별도 시작 명령 대기
