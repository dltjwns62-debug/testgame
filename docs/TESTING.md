...
```text
[ ] 게임이 정상적으로 열린다.
[ ] 몬스터 우클릭 후 전투 화면에 진입한다.
[ ] 전투 화면 진입 직후 양쪽 HP가 최대치다.
[ ] 플레이어가 약 1초 간격으로 공격한다.
[ ] 몬스터가 약 1.4초 간격으로 공격한다.
[ ] 플레이어 공격마다 몬스터 HP가 10 감소한다.
[ ] 몬스터 공격마다 플레이어 HP가 8 감소한다.
[ ] HP 텍스트가 공격 직후 갱신된다.
[ ] HP 바가 HP 비율에 맞게 감소한다.
[ ] 최근 공격 기록이 표시된다.
[ ] 공격 기록이 무한히 쌓이지 않는다.
[ ] 몬스터 HP가 0 미만으로 내려가지 않는다.
[ ] 플레이어 HP가 0 미만으로 내려가지 않는다.
[ ] 한쪽 HP가 0이 되면 모든 공격이 멈춘다.
[ ] HP가 0이 된 뒤 공격 기록이 추가되지 않는다.
[ ] Victory 또는 Defeat 결과가 표시되지 않는다.
[ ] 보상과 경험치가 지급되지 않는다.
[ ] Return to Field 버튼이 전투 도중 작동한다.
[ ] Return to Field 버튼이 전투 정지 후에도 작동한다.
[ ] 필드 복귀 후 위치와 선택 표시가 유지된다.
[ ] 필드 복귀 직후 자동 재진입하지 않는다.
[ ] 다시 전투에 들어가면 HP와 전투 로그가 초기화된다.
[ ] 브라우저 콘솔에 빨간 오류가 없다.
...
5단계 사용자 실행 테스트 상태: 미실시
5단계 ChatGPT 코드 검수: 미실시
5단계 `main` 반영: 미반영
