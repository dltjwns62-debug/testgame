...
8단계는 Auto Hunt ON/OFF, 수동 MOVE·FOCUS_ATTACK 우선순위, 지역 동료 지원, 타깃 분산, 축소된 근접 사거리와 단일 선택 공격 범위 표시를 포함한다. 현재 안정 버전은 `main`이며, 다음 9단계 스킬 기능은 아직 시작하지 않았다.
...
7단계와 8단계 사용자 실행 테스트는 통과했고 ChatGPT 코드 검수는 승인됐다. 8단계 승인 태그는 `review-stage-08-v3`이며 완료 태그는 `stage-08-completed`다.
