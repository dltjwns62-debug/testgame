...
- operation envelope, 명시 DTO, idempotent FIFO queue, record 상태·retry/backoff·rejection 보존·revision·conflict 계약을 추가했다.
- server-authoritative conflict resolver와 `OnlineSyncCoordinator`의 abort/dispose/stale response/revision guard 및 Retry-After 전달을 추가했다.
- `OnlineStatusScene`에서 Disabled/Mock 상태를 명시하고 기존 localStorage 게임을 유지한다.
- canonical snapshot·operation/queue malformed 검증, OpenAPI·TypeScript response 계약, threat model, provider 비교 문서를 보완했다.
...
- 브라우저 자동 확인: Field/Battle Stage 17 표기, 기본 `DISABLED`, `?onlineMock=1` Mock 상태, root·`/favicon.svg` HTTP 200 및 콘솔 error/warning 없음 확인 (`partial_passed`); 전체 Online Status 버튼·shutdown·재진입은 `not_run`
- 실제 서버·실제 계정·실제 네트워크: `not_run`
- 사용자 수동 테스트: 사용자 요청으로 생략 (`skipped_by_user`)
- ChatGPT 코드 검수: 승인 (`approved`)

v1·v2·v3·v4 검수는 `changes_requested` 이력과 불변 태그로 보존한다. v5는 승인되어 `main`에 반영됐고 `stage-17-completed`로 고정했다. 실제 서버·계정·네트워크·멀티플레이어와 전체 Online Status 통합 흐름은 `not_run`이며 사용자 수동 테스트는 `skipped_by_user`다.
...
- Recovery Try Again은 기존 FATAL 이슈를 명시적으로 제거하고 Recovery·SaveData·Battle·Field를 정리한 뒤 Bootstrap에서 저장 상태를 다시 검증한다.
