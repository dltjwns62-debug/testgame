...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 사용자의 별도 승인 명령 전까지 1로 유지하며, 현재 1단계는 `review_pending` 상태다.

검수 대상 버전은 tracked 파일 안의 현재 커밋 해시가 아니라 변경되지 않는 Git 태그로 관리한다. 검수 태그는 `review-stage-XX-vN` 형식을 사용하고, 기존 원격 태그는 이동하거나 덮어쓰지 않는다. 현재 검수 기준 태그는 `review-stage-01-v1`이다.

최초 저장소 생성 과정에서 검수 전에 `main`에 초기 문서 커밋이 반영된 사실은 일회성 부트스트랩 예외로 기록한다. 해당 반영은 정식 검수 승인을 의미하지 않으며, 현재 1단계는 계속 `review_pending` 상태다.
...
- `project-status.json` 생성
- 모든 문서의 단계 정보가 서로 일치함
- 검수 대상 작업 브랜치에 정상적으로 커밋됨
- 원격 작업 브랜치에 정상적으로 push됨
- 사용자 검수 통과
- 승인 후 `main`에 반영됨
...
현재는 마지막 두 조건을 충족하지 않았으므로 1단계를 완료 상태로 표시하지 않는다.
