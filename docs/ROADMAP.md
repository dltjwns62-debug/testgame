...
- `main` 반영 여부: 1단계부터 10단계까지 반영 완료

현재 단계 번호는 10이며, 10단계 v3 검수와 사용자 통합 테스트가 승인되어 `completed` 상태다. 승인된 작업은 `main`에 반영됐고 완료 태그는 `stage-10-completed`다. 다음 11단계는 별도의 시작 명령 전까지 구현하지 않는다.
...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 10이며, 1단계부터 10단계까지 `main` 반영이 완료됐다. 11단계는 별도의 시작 명령 전까지 시작하지 않는다.
...
Gold를 사용해 시험용이 아닌 용병을 구매하는 흐름을 추가한다.
