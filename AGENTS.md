...
완료된 단계는 `main`에 반영된 상태를 기준으로 한다. 다음 단계 작업 브랜치는 승인된 최신 `main`에서 생성한다. 3단계는 완료됐고, 현재 4단계는 접촉 시 전투 화면 전환만 구현한다. 검수 전에는 `main`에 반영하지 않으며, 4단계 승인 전에는 5단계를 시작하지 않는다.

4단계에서는 접촉과 전투 화면 전환만 구현한다. BattleScene의 HP는 정적인 초기 표시이며, 자동 공격·피해량·HP 감소는 5단계 전까지 구현하지 않는다. 사용자 실행 테스트는 사용자가 별도로 생략하지 않는 이상 미실시 상태로 제출한다. 다음 단계 작업은 별도 시작 명령 후 승인된 최신 `main`에서 생성한 작업 브랜치에서만 수행한다.
