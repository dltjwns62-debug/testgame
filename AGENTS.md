...
완료된 단계는 `main`에 반영된 상태를 기준으로 한다. 다음 단계 작업 브랜치는 승인된 최신 `main`에서 생성한다. 3단계는 우클릭 선택과 자동 접근 구현, 코드 검수 승인, `main` 반영까지 완료됐다. 3단계 사용자 수동 실행 테스트는 사용자의 명시적 요청으로 생략했으며 통과로 기록하지 않는다. 4단계 승인 전에는 시작하지 않는다.

3단계에서는 Phaser·TypeScript·Vite 기반의 몬스터 선택과 자동 접근만 다뤘으며, 전투 화면·BATTLE 상태·공격·체력 등 4단계 이후 기능을 임의로 추가하지 않는다. 다음 단계 작업은 별도 시작 명령 후 승인된 최신 `main`에서 생성한 작업 브랜치에서만 수행한다.
