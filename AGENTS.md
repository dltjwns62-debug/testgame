...
완료된 단계는 `main`에 반영된 상태를 기준으로 한다. 다음 단계 작업 브랜치는 승인된 최신 `main`에서 생성한다. 현재 3단계는 우클릭 선택과 자동 접근만 구현하며, 검수 전에는 `main`에 반영하지 않는다. 3단계 승인 전에는 4단계를 시작하지 않는다.

이번 단계에서는 Phaser·TypeScript·Vite 기반의 몬스터 선택과 자동 접근만 다루며, 전투 화면·BATTLE 상태·공격·체력 등 4단계 이후 기능을 임의로 추가하지 않는다. 검수 전에는 `main`에 반영하지 않으며, 승인 후에만 `main` 반영과 단계 완료 처리를 한다.
