...
완료된 단계는 `main`에 반영된 상태를 기준으로 한다. 1단계부터 6단계까지 완료됐고, 7단계는 작업 브랜치에서 구현 후 검수 대기 중이다. 검수 전에는 `main`에 반영하지 않는다. 별도의 8단계 시작 명령 전에는 8단계 기능을 구현하지 않는다.

7단계에서는 10대10 RTS 핵심 전투만 구현한다. 자동사냥, 실제 스킬, 상점, 편성 재배치, 부대 지정, 경험치·레벨·아이템·저장·온라인 기능은 구현하지 않는다. 7단계 승인 전에는 8단계를 시작하지 않으며, 기존 `favicon` 문제는 별도 작업 브랜치에서 검수한다.
