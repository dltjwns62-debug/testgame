...
## Stage 17 온라인 확장 준비 — `review_pending`
...
- 주요 구현 항목: `OnlinePlayerSnapshot`, protocol v1, Disabled/Mock Gateway, pending operation queue, idempotency, revision conflict, server-authoritative policy, sync coordinator, Online Status Scene, OpenAPI·threat model·provider 비교 문서.
...
17단계는 실제 온라인 서비스를 제공하지 않으며, `main` 병합과 프로젝트 전체 완료는 검수 승인 후에만 처리한다.
