...
## Stage 17 온라인 확장 준비 — `review_pending` (v2 재제출)
...
- 주요 구현 항목: `OnlinePlayerSnapshot`, protocol v1, Disabled/Mock Gateway, record-based pending operation queue와 retry/backoff/rejection, idempotency, revision conflict, canonical snapshot·operation validation, server-authoritative policy, lifecycle-safe sync coordinator, Online Status Scene, OpenAPI·threat model·provider 비교 문서.
...
17단계는 실제 온라인 서비스를 제공하지 않으며, `main` 병합과 프로젝트 전체 완료는 검수 승인 후에만 처리한다. v1 검수의 `changes_requested` 이력과 `review-stage-17-v1` 태그는 보존하고, 보완본을 `review-stage-17-v2`로 재제출한다.
