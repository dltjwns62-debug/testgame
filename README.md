...
8단계 v2에서는 전투 화면의 Auto Hunt ON/OFF, 전역 자동 타깃 탐색, 수동 MOVE·FOCUS_ATTACK 우선순위, 지역 동료 지원, 축소된 근접 사거리와 단일 선택 공격 범위 표시를 다룬다. 9단계 스킬과 다른 후속 기능은 아직 시작하지 않는다.
...
7단계 사용자 실행 테스트는 통과했고 ChatGPT 코드 검수는 승인됐다. 8단계 v2는 현재 `review_pending` 상태로 검수 대기 중이며, 사용자 수동 테스트와 ChatGPT 코드 검수는 아직 실시하지 않았다.
