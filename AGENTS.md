...
완료된 단계는 `main`에 반영된 상태를 기준으로 한다. 1단계부터 14단계까지 완료됐고, 15단계는 `stage-15-save-offline-progress`에서 `review_pending`으로 제출한다. 검수 전에는 `main`을 변경하거나 병합하지 않는다. 16단계 성능 및 안정화는 구현하지 않는다.
