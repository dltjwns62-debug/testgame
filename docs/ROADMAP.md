...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 2이며, 현재 2단계는 `completed` 상태다. 1단계와 2단계는 `main` 반영까지 완료됐고, 3단계는 `not_started` 상태로 유지한다. `review-stage-02-v1` 검수는 수정 요청으로 종료됐으며 `review-stage-02-v2` 검수와 main 반영으로 2단계를 완료했다.
...
1단계와 2단계의 모든 완료 조건이 충족되어 상태를 `completed`로 확정했다. 완료 기준은 `main` 반영이며, 현재 단계 번호는 2로 유지한다.
