...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 1로 유지하며, 현재 1단계는 `completed` 상태다. 별도의 2단계 시작 명령 전까지 2단계는 `not_started` 상태로 유지한다.
...
최초 저장소 생성 과정에서 검수 전에 `main`에 초기 문서 커밋이 반영된 사실은 일회성 부트스트랩 예외로 기록했다. 해당 당시 반영은 정식 검수 승인을 의미하지 않았으며, 이후 승인된 `review-stage-01-v1`을 `main`에 반영해 현재 1단계를 완료 확정했다.
...
1단계의 모든 완료 조건이 충족되어 상태를 `completed`로 확정했다. 완료 기준은 `main` 반영이며, 2단계는 아직 `not_started` 상태다.

## 2단계: 필드 화면에 플레이어와 몬스터 표시

### 단계 상태

`not_started` — 1단계 완료 후 별도 시작 명령 대기
