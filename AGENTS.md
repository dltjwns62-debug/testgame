...
완료된 단계는 `main`에 반영된 상태를 기준으로 한다. 다음 단계 작업 브랜치는 승인된 최신 `main`에서 생성한다. 1단계부터 4단계까지 완료됐고, 다음 예정 단계는 5단계 기본 자동전투다. 5단계 시작 명령 전에는 5단계를 시작하지 않는다.

4단계에서는 접촉과 전투 화면 전환만 구현했으며, BattleScene의 HP는 정적인 초기 표시다. 자동 공격·피해량·HP 감소 코드는 아직 추가하지 않는다. 5단계 작업은 별도 시작 명령 후 승인된 최신 `main`에서 생성한 작업 브랜치에서만 수행한다.
