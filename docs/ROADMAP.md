...
- 검수 승인된 단계: 1단계, 2단계, 3단계, 4단계, 5단계, 6단계, 7단계, 8단계, 9단계, 10단계
- `main` 반영 여부: 1단계부터 10단계까지 반영 완료. 11단계는 작업 브랜치에서 검수 대기

현재 단계 번호는 11이며, 11단계 구현을 `stage-11-shop-recruitment`에서 마치고 `review_pending` 상태로 제출했다. 1단계부터 10단계까지 승인 작업은 `main`에 반영됐고 완료 태그는 `stage-10-completed`다. 11단계 승인 전에는 `main`에 병합하지 않으며 다음 12단계는 시작하지 않는다.
...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 11이며, 1단계부터 10단계까지 `main` 반영이 완료됐고 11단계는 작업 브랜치에서 검수 대기 중이다. 12단계는 별도의 시작 명령 전까지 시작하지 않는다.
...
`review_pending` — `review-stage-11-v1` 제출, `main` 미반영
...
- 초기 Gold가 0이고 손상된 registry 값이 안전하게 복구된다.
- 세 상품 구매와 Gold 차감이 중복 없이 원자적으로 반영된다.
- 기본 10명과 구매 유닛 최대 3명이 검증되며 임의·중복 데이터가 거부된다.
- 구매 직후 유닛이 Bench에 남고 슬롯은 자동 변경되지 않는다.
- Reset Default가 구매 유닛을 삭제하지 않고 초기 슬롯만 복원한다.
- 배치된 구매 유닛은 최대 10명 전투 roster에 정의 능력치로 참가한다.
- `npm ci`, typecheck, build, dev 및 가능한 브라우저 검증을 완료한다.
- 사용자 수동 테스트는 요청에 따라 생략했음을 정확히 기록한다.
- ChatGPT 코드 검수 승인과 `main` 반영은 별도 후속 승인 작업으로 남긴다.
...
- 죽은 유닛 제외와 선택 상태가 부대 호출 규칙에 맞게 동작한다.
