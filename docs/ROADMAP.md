...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 2이며, 현재 2단계는 `review_pending` 상태다. 1단계는 `completed` 상태로 유지하고, 3단계는 `not_started` 상태로 유지한다.
...
`review_pending` — 구현 완료, 사용자 실행 테스트·ChatGPT 코드 검수·main 반영 대기
...
자동 검사와 브라우저 시작·렌더링 확인은 완료했지만 사용자 실행 테스트, ChatGPT 승인, `main` 반영이 끝나지 않았으므로 2단계를 `completed`로 기록하지 않는다.
