...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 4이며, 현재 4단계는 `completed` 상태다. 1단계부터 4단계까지 `main` 반영이 완료됐고, 5단계는 `not_started` 상태로 유지한다. 5단계 시작 명령 전에는 5단계를 시작하지 않는다.
