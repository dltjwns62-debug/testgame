...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 8이며, 현재 8단계는 구현 후 `review_pending` 상태다. 1단계부터 7단계까지 `main` 반영이 완료됐고, 8단계 승인 전에는 `main`에 반영하지 않는다. 별도의 9단계 시작 명령 전에는 9단계를 시작하지 않는다.
