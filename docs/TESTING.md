...
[ ] 빨간 콘솔 오류가 없다.
[ ] 몬스터를 우클릭하면 선택 표시가 나타난다.
[ ] 플레이어가 선택한 몬스터에게 이동한다.
[ ] 접촉 시 전투 화면으로 전환된다.
[ ] 전투 화면이 한 번만 열린다.
[ ] 전투 화면에 Player 이름이 표시된다.
[ ] 전투 화면에 선택한 몬스터 이름이 정확히 표시된다.
[ ] Player HP가 정적으로 표시된다.
[ ] 몬스터 HP가 정적으로 표시된다.
[ ] HP가 자동으로 감소하지 않는다.
[ ] 공격 또는 피해 메시지가 나타나지 않는다.
[ ] Return to Field 버튼이 동작한다.
[ ] 필드 복귀 후 플레이어 위치가 유지된다.
[ ] 필드 복귀 후 몬스터 위치가 유지된다.
[ ] 필드 복귀 후 기존 선택 표시가 유지된다.
[ ] 필드 복귀 직후 전투 화면에 자동 재진입하지 않는다.
...
4단계 사용자 실행 테스트 상태: 미실시
4단계 ChatGPT 코드 검수: 미실시
4단계 `main` 반영: 미반영
