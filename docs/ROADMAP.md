...
각 단계는 목표와 완료 조건을 검수한 뒤 진행한다. 현재 단계 번호는 6이며, 현재 6단계는 `review_pending` 상태다. 1단계부터 5단계까지 `main` 반영이 완료됐고, 6단계는 작업 브랜치에서 검수 대기 중이다. 6단계 승인 전에는 `main`에 반영하지 않으며, 7단계 시작 명령 전에는 7단계를 시작하지 않는다.
...
`review_pending` — 구현 완료, 검수 대기. 작업 브랜치: `stage-06-results-rewards-respawn`
